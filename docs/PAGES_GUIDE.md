...
- Shared `@/components/shared/*`:
  ...
  - `status-badge` → `StatusBadge {status}` (knows: online, available, in-use, occupied, charging, maintenance, offline, faulted, reserved, confirmed, active, pending, scheduled, completed, cancelled, expired, failed, refunded, open, assigned, in-progress, resolved, critical, high, medium, low, suspended, paid, overdue, on-duty, off-duty, on-leave). Status lookup is case- and separator-insensitive, so the SRS spellings (`RESERVED`, `IN_PROGRESS`, `ON_DUTY`) resolve to the same entry as the lower-kebab ones.
...
- Mock data in `@/data/*`: stations.js (stations, chargers, timeSlots, connectors, connectorsFor(stationId)), sessions.js (activeSession, chargingHistory, monthlyUsage, driverBookings), wallet.js (wallet, transactions), marketplace.js (products, categories), community.js (posts, leaderboard), reviews.js (reviews, ratingBreakdown(items)), reservations.js (reservations, waitlistEntries), notifications.js (notifications — filter by `roles`), fleet.js (fleetVehicles, fleetDrivers, chargingSchedule, depotPowerLimitKw, fleetEnergyByWeek, fleetCostPerVehicle, fleetUtilization, invoices, costBreakdown), tickets.js (tickets, SLA_HOURS, faultCategories), users.js (currentUsers, adminUsers), analytics.js (revenueByDay, revenueByStation, sessionsByHour, reservationsList, platformGrowth, revenueBySegment, energyMix, regionPerformance). READ the data file before using it — match field names exactly.
- Entities that come straight from the SRS keep the SRS vocabulary, upper snake case: `Connector.status` AVAILABLE/RESERVED/OCCUPIED/FAULTED, `Reservation.status` RESERVED/ACTIVE/EXPIRED/CANCELLED, `FaultTicket.status` OPEN/ASSIGNED/IN_PROGRESS/RESOLVED with priority LOW/MEDIUM/HIGH/CRITICAL and source DRIVER_REPORT/KIOSK_EMULATOR/PM_SCHEDULE/FIELD_INSPECTION. The older lowercase vocabulary (`chargers`, `fleetVehicles`) is unchanged — do not mix the two on one entity.
- `@/lib/kiosk-emulator`: the Software-based Kiosk Telemetry Emulator Engine (SRS §2.6, §4.2) — `FAULT_CODES`, `KIOSK_STATES`, `canTransition`, and `createKioskEmulator({connectorId, powerKw, targetKwh, tickMs, faultRate, seed})` which streams `{kioskState, plugDetected, chargingKwh, faultState}` frames to `subscribe(fn)`. It stands in for the WebSocket charger feed; the frame shape is the contract real hardware will implement.
- `@/lib/nav`: `NAV` (sidebar per role), `ROLE_META`, and `SRS_MODULES` — the SRS §2.2 module → screen traceability map rendered by the public `/modules` page. Add new screens to both `NAV` and `SRS_MODULES`.
